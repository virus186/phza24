...
<div class="primary_input mb-25">
    ...
    <select name="unit_type_id" id="unit_type_id" class="primary_select mb-15 unit"
        ...
        @foreach ($units as $key => $unit)
            <option value="{{ $unit->id }}" @if(old('unit_type_id')) @if (old('unit_type_id') == $unit->id) selected @endif @else @isset($product) @if ($product->unit_type_id == $unit->id) selected @endif @endisset @endif>{{ $unit->name }}</option>
        @endforeach
    </select>
